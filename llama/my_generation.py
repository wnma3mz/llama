...
class LLaMA:
    ...
    def train(
        ...
        tokens = input_ids.to(local_rank)

        logits = self.model(tokens, start_pos)

        loss_fn = nn.CrossEntropyLoss()
        shift_logits = logits[..., :-1, :].contiguous()
        shift_labels = labels[..., 1:].contiguous()

        shift_logits = shift_logits.view(-1, self.tokenizer.n_words)
        shift_labels = shift_labels.view(-1)
        loss = loss_fn(shift_logits, shift_labels.to(shift_logits.device))
        print(loss)
        return loss
