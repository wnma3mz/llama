...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.rotary_emb = LlamaRotaryEmbedding(self.head_dim, max_position_embeddings=args.max_seq_len)
    ...
    def forward(
        ...
        xq, keys = apply_rotary_pos_emb(xq, xk, cos, sin, position_ids)
        values = xv
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        h = x + self.attention(self.attention_norm(x), position_ids, mask, start_pos)
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = torch.arange(
                past_key_values_length, seqlen + past_key_values_length, dtype=torch.long, device=h.device,
            )
            position_ids = position_ids.unsqueeze(0).view(-1, seqlen)

        for layer in self.layers:
            h = layer(h, position_ids, mask)
        h = self.norm(h)
